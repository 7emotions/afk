...
const tmp = mkdtempSync(join(tmpdir(), "afk-daemon-auth-"))
...
before(async () => {
  ...
  const built = createHttpServer(registry, pendingStore, { secret: SECRET, modeStore })
  server = built.server
...
test("POST /register returns 429 when the registry is at its cap", async () => {
  ...
  const pendingStore = createPendingStore({ path: join(tmp, "cap-pending.json") })
  const modeStore = createModeStore({ path: join(tmp, "cap-mode.json") })
  const built = createHttpServer(reg, pendingStore, { secret: SECRET, modeStore })
